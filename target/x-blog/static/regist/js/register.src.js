	...
	function require(options){
		...
		this.rules={			
				isEmail: function(value, errorMsg) {
					   ...
					   errorMsg=errorMsg||" ";   if (!rule.email.test(value)) return errorMsg;
					   },
	...
	require.prototype={
		...
		_change:function($this){			
			...
			if($this.val().length!=11)
			{
				$("#verifyYz").hide();
				$("#time_box").show();
				if(timerC===300) $("#time_box").text("发送验证码");
				$("#verifyNo").val("");
